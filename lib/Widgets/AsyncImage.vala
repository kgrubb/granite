 ...
 * and provides with two main methods: {@link Granite.AsyncImage.set_from_gicon_async}
 * and {@link Granite.AsyncImage.set_from_file_async}.
 *
 * AsyncImage internally operates only on {@link Gdk.Pixbuf} and
 * {@link Cairo.Surface}'s which means that you cannot read valid properties from
 * the main {@link Gtk.Image} like {@link Gtk.Image.icon_name},
 * {@link Gtk.Image.gicon} or {@link Gtk.Image.file}.
 * The only property which will be set is the final surface: {@link Gtk.Image.surface}.
 *
 * Even though AsyncImage sets only the {@link Gtk.Image.surface}, it automatically
 * detects changes to the underlying {@link Gtk.Widget.scale_factor} and reloads
 * the icon to a new scale factor when it changes. If you request to set an
 * {@link GLib.ThemedIcon} and the icon or GTK theme changes the AsyncImage will
 * also reload it to display the new icon with applied changes.
 *
 * The {@link Granite.AsyncImage.gicon_async} and
 * {@link Granite.AsyncImage.size_async} are properties which reflect the current
 * icon and it's size which will or is currently displayed. Note that those two
 * properties will return meaningful results //''only''// when you call
 * {@link Granite.AsyncImage.set_from_gicon_async} and it's wrappers.
 *
 * AsyncImage has also its own cache for already loaded icons. If you attempt to
 * load the same icon at the same size AsyncImage will look it up and if it's
 * available, will set it immediately.
 *
 * If you want to detect when the image was actually loaded into the
 * {@link Cairo.Surface} you can connect to the {@link GLib.Object.notify} signal
 * for {@link Gtk.Image.surface}.
 */
public class Granite.AsyncImage : Gtk.Image {
     ...
     *
     * Note that this property is by default and will be ``null`` if you didn't
     * call the {@link Granite.AsyncImage.set_from_gicon_async} or it's wrappers.
     */
     ...
     *
     * Note that this property is by default and will be ``-1`` if you didn't
     * call the {@link Granite.AsyncImage.set_from_gicon_async} or it's wrappers.
     */
     ...
     * Internally this parameter causes the {@link Granite.AsyncImage} to connect to the
     * {@link Gtk.Widget.realize} signal.
     *
     ...
     * call the {@link Gtk.Widget.set_size_request} with the passed ``size`` or ``width`` and
     * ``height`` if you called {@link Granite.AsyncImage.set_from_file_async}.
     * When image is loaded and shown the size request is then reset to the original values.
     *
     * @param load_on_realize if ``true`` the image will be loaded when it's rendered, false to
     ...
     *
     * This is equivalent to calling {@link Granite.AsyncImage.AsyncImage} and
     * {@link Granite.AsyncImage.set_from_gicon_async}.
     *
     * @param icon the {@link GLib.Icon} to display in the image
     * @param size the size of the icon, ``-1`` to load the default size
     * @param load_on_realize if ``true`` the image will be loaded when it's rendered,
     * false to load the image immediately
     * @param auto_size_request if the widget should act as a placeholder when the image is not yet loaded
     */
     ...
     *
     * This is equivalent to calling {@link Granite.AsyncImage.AsyncImage} and
     * {@link Granite.AsyncImage.set_from_icon_name_async}.
     *
     ...
     * from an icon name.
     *
     * If the ``icon`` is a {@link GLib.FileIcon} then the image will be loaded using
     * the {@link Granite.AsyncImage.set_from_file_async} method with the supplied size
     * for both ``width`` and ``height`` with preserving the aspect ratio of the image.
     *
     * If the {@link Granite.AsyncImage.load_on_realize} is ``true``, the error will never
     * be thrown in this method since the loading will happen internally in the AsyncImage
     * when the {@link Gtk.Widget.realize} signal is invoked. In this case, a warning will
     * be printed with relevant information about a fauilure.
     *
     * @param icon the {@link GLib.Icon} to display in the image
     * @param size the size of the icon, ``0`` will clear the {@link Gtk.Image.pixbuf},
     * ``-1`` to load the default size
     ...
     *
     * This is a convenience method for setting an icon name with a desired {@link Gtk.IconSize}.
     * Note that you'll not be able to change the icon size afterwards with {@link Gtk.Image.pixel_size}
     * or {@link Gtk.Image.icon_size}. You will have to call one of the {@link Granite.AsyncImage}
     * set_from_ methods to change it's size.
     *
     ...
     *
     * ''Note that this method is not a wrapper to the main''
     * {@link Granite.AsyncImage.set_from_gicon_async} ''method''. Internally, it only creates a
     * {@link Gdk.Pixbuf} with an {@link GLib.InputStream}, loads it asynchronously
     * and sets the {@link Gtk.Image}'s surface to the result.
     *
     * This method will reset the {@link Granite.AsyncImage.gicon_async} and
     * {@link Granite.AsyncImage.size_async} properties to their default values
     * and will not make the {@link Granite.AsyncImage} update the image when the
     * scale factor or icon theme changes.
     *
     * For the time that the image is loaded, the size request of the AsyncImage
     * will be set to ``width`` and ``height`` if ``auto_size_request`` is set to ``true``
     *
