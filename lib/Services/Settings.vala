...
namespace Granite.Services {
     ...
     *
     * For example, if a developer wanted to interact with desktop.Wallpaper's
     * (http:/www.launchpad.net/pantheon-wallpaper) schema, this is what his/her
...
*
     *  1. Any subclass looks at all properties it contains, and loads their initial
     *     values from the keys they represent. Because Vala properties are stored
     *     as GLib properties, the string representation of a property replaces underscores with
     *     hyphens (i.e. property_name becomes "property-name"). This is how this library knows
     *     which keys to load from. If the key does not exist, it will result in a fatal error.
     *  1. When a property of the subclass changes, the library will first verify the data before
     *     emitting a changed signal. If necessary, the library will change the value of the property
     *     while verifying. This is why developers should only act upon emissions of the changed ()
     *     signal and never the native {@link GLib.Object.notify} signal.
     *  1. When the corresponding key of one of the properties of the subclass changes, it will also
     *     verify the data and change it, if necessary, before loading it into as the corresponding property's value.
     */
    public abstract class Settings : GLib.Object {
        ...
        /**
         * Creates a new {@link Granite.Services.Settings} object for the supplied
         * schema and {@link GLib.SettingsBackend}.
         *
         * @param schema the name of the schema to interact with
        ...
        /**
         * Creates a new {@link Granite.Services.Settings} object for the supplied schema,
         * {@link GLib.SettingsBackend}, and path.
         *
         * @param schema the name of the schema to interact with
         * @param backend the desired backend to use
